...
function renderTab(tabName) {
    ...
    const concertEntries = Object.entries(r.datesLocations || {}).sort((a, b) => {
        const dateA = a[1][0]
        const dateB = b[1][0]
        const toISO = d.split('-').reverse().join('-');
...
searchBar.addEventListener('keyup', (e) => {
    ...
    renderArtists({ fiter: activeFilter, query: e.target.value});
});
...
function renderAdmin() {
    ...
    artistsAdmin.innerHTML = artists.map(a => `
        <div style="display:flex;gap:8px;align-items:center;margin-bottom:8px">
            <img src="${a.image}" style="width:48px;height:48px;object-fit:cover;border-radius:6px"
...
function acceptRequest(reqId) {
    ...
    const customArtists = JSON.parse(localStorage.getItem('hp_custom_arstists') || '[]');
    customArtists.push(newArtist);
    requests = requests.filter(r => r.id !== reqId);
    saveRequests(requests);
    alert(`${req.name} ajouté(e) !`);
    renderAdmin();
    renderArtists({ filter: document.querySelector('genre-btn.active')?.dataset.filter || 'all', query: searchBar.value });
